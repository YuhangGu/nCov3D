...
(function () {
    ...
    app.init = function (container, width_p, height_p) {
    ...
    app.saveCanvasImage = function (width, height, fill_background) {
        ...
        var saveCanvasImage = function (canvas) {
            console.log("here saveCanvasImage");
...
Q3D.DEMLayer.prototype.segmentizeLineString = function (lineString, zFunc) {
    ...
    if (zFunc === undefined) zFunc = function () {
        return 0;
    };
...
Q3D.PointLayer.prototype.build = function (parent) {
    if (this.objType == "Icon") {
        this.buildIcons(parent);
        return;
    }
    if (this.objType == "JSON model" || this.objType == "COLLADA model") {
        this.buildModels(parent);
        return;
    }
    ...
    if (this.objType == "Sphere") createGeometry = function (f) {
        return new THREE.SphereGeometry(f.r);
    };
    else if (this.objType == "Box") createGeometry = function (f) {
    ...
    else createGeometry = function (f) {
            return new THREE.CylinderGeometry(f.rt, f.rb, f.h);
        };   // Cylinder or Cone
...
Q3D.PointLayer.prototype.buildLabels = function (parent, parentElement) {
    Q3D.VectorLayer.prototype.buildLabels.call(this, parent, parentElement, function (f) {
        return f.pts;
    });
};
...
Q3D.LineLayer.prototype.build = function (parent) {
    ...
    if (this.objType == "Line") {
        ...
        var createObject = function (f, line) {
            var bzFunc, vertices;
            if (bRelativeToDEM) bzFunc = function (x, y) {
                return dem.getZ(x, y) + f.bh;
            };
            else bzFunc = function (x, y) {
                return z0 + f.bh;
            };

            if (relativeToDEM) {
                var zFunc = function (x, y) {
                    return dem.getZ(x, y) + f.h;
                };
...
Q3D.PolygonLayer.prototype.build = function (parent) {
    ...
    if (this.objType == "Extruded") {
        var createSubObject = function (f, polygon, z) {
            ...
            var newMtr =  materials[f.m].m.clone();
            newMtr.setValues({color:f.color });

            //var mesh = new THREE.Mesh(geom, materials[f.m].m);
            var mesh = new THREE.Mesh(geom, newMtr);
        ...
        var createObject = function (f) {
            ...
            var zFunc;
            if (relativeToDEM) zFunc = function (x, y) {
                return dem.getZ(x, y) + f.h;
            };
            else zFunc = function (x, y) {
                return z0 + f.h;
            };
            ...
            if (sbRelativeToDEM) bzFunc = function (x, y) {
                return dem.getZ(x, y) + f.sb;
            };
            else bzFunc = function (x, y) {
                return z0 + f.sb;
            };
...
Q3D.PolygonLayer.prototype.update = function (parent) {
...
Q3D.PolygonLayer.prototype.buildLabels = function (parent, parentElement) {
    var zFunc, getPointsFunc = function (f) {
        return f.centroids;
    };
    var relativeToDEM = (this.am == "relative");    // altitude mode
    if (relativeToDEM) {
...
Q3D.Utils.putStick = function (x, y, zFunc, h) {
    ...
    if (zFunc === undefined) {
        zFunc = function (x, y) {
            return Q3D.application.project.layers[0].getZ(x, y);
        }
    }
    var z = zFunc(x, y);
    var geom = new THREE.Geometry();
